...
from PyQt5.QtGui import QPixmap, QColor, QPen, QBrush, QLinearGradient
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # ── Background dinámico (generado con código en lugar de imagen PNG) ────
        # Genera un fondo personalizable sin dependencia de archivo externo.
        # NOTA: NO usamos .lower() porque esto pondría el background DETRÁS
        # de los botones. En su lugar, los botones tienen backgrounds semi-transparentes
        # que dejan ver el fondo dinámico detrás.
        background = QLabel(self)
        background.setPixmap(self._draw_background())
        background.setGeometry(0, -30, 1920, 1080)
        # background.lower()  # ← COMENTADO: esto ponía el fondo atrás de los botones
        # No llamamos a lower() para que el background sea visible
        ...
        # Version label
        VersionLabel = QLabel('v4 — IP RPI — Dynamic Improved Background — March 2026', self)
    ...
    def _draw_background(self):
        """
        Genera fondo realista para sala de conferencias.
        VERSIÓN FINAL: Gradiente gris + asientos + podio + decoración
        """
        try:
            # Crear pixmap
            ...
            # ────────────────────────────────────────────────────────────────
            #  GRADIENTE: Superior claro → Inferior oscuro (realismo)
            # ────────────────────────────────────────────────────────────────
            gradient = QLinearGradient(0, 0, 0, 1080)
            gradient.setColorAt(0.0, QColor(160, 160, 165))
            gradient.setColorAt(0.5, QColor(120, 120, 125))
            gradient.setColorAt(1.0, QColor(80, 80, 85))
            painter.fillRect(0, 0, 1920, 1080, gradient)
            
            # ────────────────────────────────────────────────────────────────
            #  ESCENARIO (superior, fondo más claro)
            # ────────────────────────────────────────────────────────────────
            painter.fillRect(0, 0, 1920, 140, QColor(150, 150, 155))
            
            # Línea separadora escenario/público
            painter.setPen(QPen(QColor(80, 80, 85), 2))
            painter.drawLine(0, 140, 1920, 140)
            
            # PODIO (centro con madera oscura)
            podium_rect = QtCore.QRect(860, 20, 200, 80)
            painter.fillRect(podium_rect, QColor(110, 90, 70))
            painter.setPen(QPen(QColor(50, 40, 30), 3))
            painter.drawRect(podium_rect)
            
            # Símbolo presidencia
            painter.setPen(QColor(200, 180, 100))
            painter.setFont(QtGui.QFont("Arial", 32, QtGui.QFont.Bold))
            painter.drawText(podium_rect, QtCore.Qt.AlignCenter, "⚜️")
            
            # Etiquetas plataforma
            painter.setPen(QColor(40, 40, 50))
            painter.setFont(QtGui.QFont("Arial", 11, QtGui.QFont.Bold))
            painter.drawText(860, 105, 200, 30, QtCore.Qt.AlignCenter, "CHAIRMAN")
            painter.drawText(400, 105, 150, 30, QtCore.Qt.AlignCenter, "LEFT")
            painter.drawText(1370, 105, 150, 30, QtCore.Qt.AlignCenter, "RIGHT")
            
            # ────────────────────────────────────────────────────────────────
            #  SECCIONES DE PÚBLICO (asientos como círculos pequeños)
            # ────────────────────────────────────────────────────────────────
            def draw_seats(x_start, y_start, rows, cols, label):
                """Dibuja una sección de asientos con círculos visuales."""
                seat_r = 4
                sx, sy = 50, 50
                
                # Marco sección
                painter.setPen(QPen(QColor(110, 110, 115), 1, QtCore.Qt.DashLine))
                painter.drawRect(x_start - 15, y_start - 15,
                               cols * sx + 30, rows * sy + 30)
                
                # Asientos (círculos azules pequeños)
                painter.setPen(QPen(QColor(60, 100, 140), 1))
                painter.setBrush(QBrush(QColor(100, 150, 180)))
                
                for r in range(rows):
                    for c in range(cols):
                        x = x_start + c * sx
                        y = y_start + r * sy
                        painter.drawEllipse(x - seat_r, y - seat_r,
                                          seat_r * 2, seat_r * 2)
                
                # Etiqueta sección
                painter.setPen(QColor(170, 170, 175))
                painter.setFont(QtGui.QFont("Arial", 10, QtGui.QFont.Bold))
                painter.drawText(x_start - 15, y_start - 35, 100, 20,
                               QtCore.Qt.AlignCenter, label)
            
            # Dibujar 3 secciones de público
            draw_seats(100, 180, 6, 4, "IZQ")
            draw_seats(650, 180, 6, 5, "CENTRO")
            draw_seats(1200, 180, 6, 4, "DER")
            
            # ────────────────────────────────────────────────────────────────
            #  BACK ROWS (filas traseras 7-9)
            # ────────────────────────────────────────────────────────────────
            painter.setPen(QColor(140, 140, 145))
            painter.setFont(QtGui.QFont("Arial", 9, QtGui.QFont.Bold))
            painter.drawText(700, 745, 500, 25, QtCore.Qt.AlignCenter, "BACK ROWS")
            
            painter.setPen(QPen(QColor(80, 120, 150), 1))
            painter.setBrush(QBrush(QColor(110, 150, 180)))
            
            for x in range(200, 1700, 40):
                painter.drawEllipse(x - 4, 800 - 4, 8, 8)
                painter.drawEllipse(x - 4, 860 - 4, 8, 8)
            
            # ────────────────────────────────────────────────────────────────
            #  ZONAS ESPECIALES
            # ────────────────────────────────────────────────────────────────
            painter.setPen(QColor(170, 140, 100))
            painter.setFont(QtGui.QFont("Arial", 12, QtGui.QFont.Bold))
            painter.drawText(100, 980, 150, 40, QtCore.Qt.AlignCenter, "♿\nWHEEL")
            painter.drawText(1650, 980, 150, 40, QtCore.Qt.AlignCenter, "🖥️\nREMOTE")
            ...
            print(f"[ERROR] _draw_background(): {e}")
            import traceback
            traceback.print_exc()
            fallback = QPixmap(1920, 1080)
            fallback.fill(QColor(100, 100, 100))
            return fallback
