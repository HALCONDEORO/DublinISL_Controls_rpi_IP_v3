...
# ptz/visca/worker.py — Worker de envío de comandos VISCA
#
            ...
            if _succeeded:
                if cmd.on_success:
                    try:
                        ...
                        logger.error("CameraWorker %s: on_success raised: %s", self.ip, exc)
            else:
                if cmd.on_failure:
                    try:
                        cmd.on_failure()
                    except Exception as exc:
                        logger.error("CameraWorker %s: on_failure raised: %s", self.ip, exc)